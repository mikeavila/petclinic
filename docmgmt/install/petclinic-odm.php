...
include "../../install/password.txt";
$dsn = "mysql:host=localhost; dbname=petclinicdocmgmt;";
try {
...
"INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'admin', '1', 'This variable sets the root user id.  The root user will be able to access all files and have authority for everything.', 'num|req');",
"INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'dataDir', '/pcmsuser/docmgmt/data/', 'location of file repository. This should ideally be outside the Web server root. Make sure the server has permissions to read/write files to this folder!. (Examples: Linux - /var/www/document_repository/ : Windows - c:/document_repository/', 'maxsize=255');",
...
"INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'file_expired_action', '4', 'Choose an action option when a file is found to be expired The first two options also result in sending email to reviewer  (1) Remove from file list until renewed (2) Show in file list but non-checkoutable (3) Send email to reviewer only (4) Do Nothing', 'num');",
"INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'authorization', 'False', 'True or False. If set True, every document must be reviewed by an admin before it can go public. To disable set to False. If False, all newly added/checked-in documents will immediately be listed', 'bool');",
...
"INSERT INTO `{$dbprefix}settings` VALUES(NULL, 'base_url', 'http://localhost/petclinic', 'Set this to the url of the site. No need for trailing \"/\" here', 'url');",
