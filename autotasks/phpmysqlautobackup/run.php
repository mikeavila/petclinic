...
// added for use with petclinic
$headerTitle = "";
include "../../includes/common.inc";
$db_server = "localhost"; // your MySQL server - localhost will normally suffice
//$mysql_username = $user;  // your MySQL username
//$mysql_password = mc_decrypt($password, p_key);  // your MySQL password
//$password=mc_decrypt($password, p_key);
$daytime_array = getdate();
//print_r($daytime_array);
switch ($daytime_array["weekday"]) {
	case "Sunday":
		if ($daytime_array["hours"] > 0 and $daytime_array["hours"] < 3)
			$db = "petclinic";
		if ($daytime_array["hours"] > 3 and $daytime_array["hours"] < 6)
			$db = "petcliniccorp";
		break;
	case "Monday":
		if ($daytime_array["hours"] > 0 and $daytime_array["hours"] < 3)
			$db = "petclinicdocmgmt";
		if ($daytime_array["hours"] > 3 and $daytime_array["hours"] < 6)
			$db = "petclinicapptboard";
		break;
	case "Tuesday":
		if ($daytime_array["hours"] > 0 and $daytime_array["hours"] < 3)
			$db = "petclinicapptgroom";
		if ($daytime_array["hours"] > 3 and $daytime_array["hours"] < 6)
			$db = "petclinicapptclinic";
		break;
	case "Wednesday":
		if ($daytime_array["hours"] > 0 and $daytime_array["hours"] < 3)
			$db = "petclinicinv";
		if ($daytime_array["hours"] > 3 and $daytime_array["hours"] < 6)
			$db = "petclinicinvoices";
		break;
	case "Thursday":
		if ($daytime_array["hours"] > 0 and $daytime_array["hours"] < 3)
			$db = "petclinicmsgs";
...
exit();
$mysql_username = "stemp";
$mysql_password = "ptemp";
...
$time_interval=3600*24;// 3600 = one hour - only allow the backup to run once each hour
...
define('LOCATION', $_SERVER['DOCUMENT_ROOT']."dbbackup");
